...
let AuthService = class AuthService {
    ...
    async loginPengendara(loginUserDto) {
        ...
        if (user) {
            ...
            if (checkEmail) {
                ...
                throw new common_1.BadRequestException("Please verify your email first, we already send verification link on your email");
            }
        }
    }
    ...
    async loginAdmin(loginAdminDto) {
        ...
        if (admin.role != `Admin`) {
            throw new common_2.ConflictException("you don't have access");
        }
        if (admin) {
            const payload = {
                sub: admin.id
    ...
    async refreshAccessToken(refreshTokenDto) {
        ...
        const refreshToken = await this.refreshTokenRepository.findOne(payload.jwt_id, { relations: ['user'] });
        if (!refreshToken) {
